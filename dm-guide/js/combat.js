...
export function renderCombatPanel() {
  ...
  let immunityLabel = 'Immune to 3rd level and below';
  let unlockedSpells = 'Physical attacks only';
  if (braziersOut >= 4) { immunityLabel = 'No spell immunity'; unlockedSpells = 'Everything works \u2014 full arsenal'; }
  else if (braziersOut === 3) { immunityLabel = 'Immune to cantrips only \u00B7 Ritual fails'; unlockedSpells = "1st-level+: Command, Tasha's Hideous Laughter"; }
  else if (braziersOut === 2) { immunityLabel = 'Immune to 1st level and below'; unlockedSpells = '2nd-level+: Hold Person, Suggestion, Heat Metal'; }
  else if (braziersOut === 1) { immunityLabel = 'Immune to 2nd level and below'; unlockedSpells = '3rd-level: Counterspell, Fireball, Dispel Magic, Spirit Guardians'; }

  const lockeHpPct = Math.max(0, c.locke.hp / c.locke.maxHp * 100);
  const lockePhase = c.locke.hp <= 55 ? 2 : 1;
  const lockeHpClass = lockeHpPct > 50 ? 'healthy' : lockeHpPct > 25 ? 'wounded' : 'critical';
  ...
  temp.innerHTML =
    ...
    c.cultFanatics.map((cf, i) => {
      ...
      const cls = pct > 50 ? 'healthy' : pct > 25 ? 'wounded' : 'critical';
